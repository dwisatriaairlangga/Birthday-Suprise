...
import { MailOpen, Gift, RefreshCcw, Droplet, Ribbon, Flower2, Stamp, Key, Feather, Sparkles, Heart, Paperclip, Star, Smile, Lock, Music, Play, Pause, Download } from 'lucide-react';
...
const WavyBottom = ({ accentColor }: { accentColor: string }) => (
  <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-none rotate-180">
    <svg className="relative block w-[calc(100%+1.3px)] h-[35px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
      ...
      <path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" fill={accentColor} opacity=".5"></path>
    </svg>
  </div>
);
...
const CardDoodles = () => (
  <>
    ...
    <div className="absolute bottom-16 right-6 text-xl opacity-40 transform rotate-45">✨</div>
    <div className="absolute top-1/2 left-3 text-xl opacity-40 transform -rotate-12">〰️</div>
  </>
);

function TulipVector({ className }: { className?: string }) { return (<svg viewBox="0 0 100 180" className={className} fill="none"><path d="M50 180 C50 140 48 100 50 70" stroke="#4A7C39" strokeWidth="4" strokeLinecap="round"/><path d="M50 140 C30 130 10 110 5 90 C15 110 35 125 50 140" fill="#5D9B47"/><path d="M50 120 C65 110 85 95 90 75 C80 95 65 110 50 120" fill="#7C9D70"/><path d="M50 70 C30 65 25 35 40 20 C45 15 50 25 50 35 C50 45 45 60 50 70" fill="#FFB6C1"/><path d="M50 70 C38 60 35 38 45 25 C48 20 50 30 50 40 C50 50 48 60 50 70" fill="#FFC0CB"/></svg>); }
...
export default function LetterPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const downloadSurat = async () => {
    ...
    if (!element) return;
    try {
      const canvas = await html2canvas(element, { useCORS: true, backgroundColor: themes[data.theme as keyof typeof themes].bg });
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        ...
        {!loadingScreen && !isLocked && (
          <motion.div key="main-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full flex flex-col items-center justify-center">
            ...
            {/* TAHAP 2: SCRAPBOOK CARDS (DIBUNGKUS div id="surat-content" UNTUK DOWNLOAD) */}
            <AnimatePresence>
              {stage === 'letter' && (
                <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-lg mx-4 md:mx-0 space-y-6 relative z-20">
                  <div id="surat-content" className="space-y-6 pb-6 pt-4 px-2" style={{ backgroundColor: activeTheme.bg }}>
                          ...
                          {paragraphs.map((para: string, i: number) => (
                            <div key={i} className="mb-4">
                              ...
                              {data.photoLayout === 'inline' && data.photos[i] && (
                                <div className={`mt-4 mx-auto bg-white p-2 pb-6 shadow-md border border-gray-100 w-48 relative transform ${i%2===0 ? 'rotate-2' : '-rotate-3'}`}>
                                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-3 bg-white/50 backdrop-blur-sm border border-gray-200 shadow-sm" />
                                  <img src={data.photos[i]} alt="Kenangan" className="w-full h-auto aspect-square object-cover" />
                                </div>
                              )}
                            </div>
                          ))}
                        ...
                        <div className="relative p-6 md:p-10 rounded-2xl shadow-xl overflow-hidden border border-black/5" style={{ backgroundColor: activeTheme.envelope }}>
                          ...
                          <div className="relative z-10 flex flex-col items-center">
                            {data.photoLayout === 'photobooth' && (
                              <div className="bg-white p-3 shadow-lg rounded-sm border border-gray-100 w-full max-w-[200px]">
                                {data.photos.map((src: string, i: number) => (
                                  <img key={i} src={src} className="w-full aspect-square object-cover mb-2 last:mb-0 bg-gray-100" />
                                ))}
                                <p className="font-serif text-center text-gray-400 text-xs mt-3 uppercase tracking-widest">Photobooth</p>
                              </div>
                            )}
                            {data.photoLayout === 'polaroid' && (
                              <div className="flex flex-wrap justify-center gap-4">
                                {data.photos.map((src: string, i: number) => (
                                  <div key={i} className={`bg-white p-2 pb-8 shadow-md border border-gray-100 w-36 sm:w-40 relative transform ${i%2===0 ? 'rotate-3' : '-rotate-2'}`}>
                                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-3 bg-white/50 backdrop-blur-sm border border-gray-200 shadow-sm" />
                            ...
                            {data.photoLayout === 'elegant' && (
                              <div className="flex flex-col gap-5 w-full items-center">
                                {data.photos.map((src: string, i: number) => (
                                  <div key={i} className="p-2 bg-[#fdfbf7] border border-gray-300 shadow-sm w-full max-w-sm">
                      ...
                      {/* KARTU 3: PLAYLIST MUSIK KESUKAAN */}
                        ...
                        <div className="relative p-6 md:p-10 rounded-2xl shadow-xl overflow-hidden border border-black/5" style={{ backgroundColor: activeTheme.envelope }}>
                          ...
                          <div className="relative z-10 flex flex-col gap-4">
                            {data.wallMessages.map((msg: any, i: number) => (
                              <div key={i} className={`bg-white/95 p-4 shadow-sm border border-gray-100 transform ${i%2===0 ? 'rotate-1' : '-rotate-1'} rounded-xl`}>
                                <p className="text-gray-700 font-serif text-sm md:text-base leading-relaxed mb-2">"{msg.message}"</p>
                      ...
                      {/* KARTU 5: PENGIRIM */}
                      <div className="relative p-6 md:p-10 rounded-2xl shadow-xl overflow-hidden text-center border border-black/5" style={{ backgroundColor: activeTheme.envelope }}>
                        <p className={`font-serif text-sm text-white/80 uppercase tracking-widest mb-1`}>Tertanda,</p>
                        <p className={`font-serif text-2xl font-black text-white tracking-wide drop-shadow-md mb-8`}>{data.sender}</p>
                        <WavyBottom accentColor={activeTheme.accent} />
                      </div>
                  </div>
      ...
      {/* PEMUTAR MUSIK MULTI-PLATFORM (TETAP MENGAMBANG) */}
